...
struct Libc::Socket_fs::Context : Plugin_context
{
	...
	private:
		...
		/* TODO transform into VFS handles */
		struct
		{
			char const      *name;
			int              num;
...
extern "C" int socket_fs_accept(int libc_fd, sockaddr *addr, socklen_t *addrlen)
{
	...
	auto _accept_fd = make_unconfirmed([&] {
		if (!accept_fd) raw("accept_fd == nullptr");
		if (!file_descriptor_allocator()) raw("file_descriptor_allocator() == nullptr");
...
extern "C" int getifaddrs(struct ifaddrs **ifap)
{
	...
	*ifap = &ifaddr;
	...
	auto read_ifaddr_file = [] (sockaddr_in &sockaddr, Socket_fs::Absolute_path const &path)
	{
		Host_string address;
		Port_string service;
		*service.base() = '0';
	...
	int err = read_ifaddr_file(address, Absolute_path("address", root.base()));

	if (!err)
		err = read_ifaddr_file(netmask, Absolute_path("netmask", root.base()));
                              ...
                              timeval *timeout)
{
	...
	for (int fd = 0; fd < nfds; ++fd) {
		...
		if (FD_ISSET(fd, &in_readfds)) {
			...
			if (fdo && (fdo->plugin == this)) try {
				Socket_fs::Context *context = static_cast<Socket_fs::Context *>(fdo->context);
